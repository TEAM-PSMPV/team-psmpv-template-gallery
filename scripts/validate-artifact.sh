...
assets="${SITES_PROJECT_ROOT}/dist/client"
...
[[ -d "${assets}" ]] || {
  echo "Missing generated static assets: dist/client" >&2
  exit 66
}
...
if (wrangler.main !== "index.js" || wrangler.assets?.directory !== "../client") {
  throw new Error("generated Wrangler config does not target the expected Worker and assets");
}
...
echo "Validated Cloudflare artifact: Worker, Wrangler config, and static assets are present."
